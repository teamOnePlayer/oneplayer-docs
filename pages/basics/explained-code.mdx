...
if (command === 'volume') {
  ...
  if (isNaN(vol) || vol < 0 || vol > 200) {
    return message.reply(`🔊 Current: ${player.volume}%`);
  }
...
if (command === '8d') {
  ...
  message.reply('🎧 8D audio enabled!');
}
...
client.on('messageCreate', async (message) => {
  ...
  if (cmd === 'play') {
    ...
    if (!result.tracks.length) return message.reply('❌ Not found!');
